...
进项目目录：
```bash
cd /Users/spin/SpinCC/Proj/feishu-getnote/worker
```
...
2. 右上角打开「开发者模式」。
3. 点「加载已解压的扩展程序」，选 `/Users/spin/SpinCC/Proj/feishu-getnote/extension/` 目录。
...
### C3. 使用
1. 浏览器打开任意飞书文档（`https://xxx.feishu.cn/docx/...` 或 `/wiki/...`）。
2. 点工具栏插件图标 → 「导入到 Get 笔记」。
3. 等 2-5 秒，提示 "✓ 已导入：<标题>"。
4. 打开 Get 笔记客户端，新笔记已经在里面（带 `from-feishu` 标签）。
